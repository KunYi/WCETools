...
#include <CommCtrl.h>
#include "UpdateUUID.h"              // Program-specific stuff
#include "nand.h"
#include "resource.h"
...
// Global data
//
const TCHAR szAppName[] = TEXT ("UpdateUUID");
UCHAR g_TOC[4096];
const PTOC g_pTOC = (PTOC)g_TOC;
const int cnUUIDlen = 13;
...
BOOL DoInitDlgMain (HWND hWnd, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    SendDlgItemMessage (hWnd, IDC_EDIT1, EM_SETLIMITTEXT, cnUUIDlen, 0);
    return 0;
}
//----------------------------------------------------------------------
...
//----------------------------------------------------------------------
//
LPARAM DoUpdateUUID (HWND hWnd, WORD idItem, HWND hwndCtl, WORD wNotifyCode)
{
	const int nShift = 16 - cnUUIDlen;
	TCHAR szString[64];
	UCHAR szUUID[17], deviceId[16];
    DWORD i;

    GetDlgItemText (hWnd, IDC_EDIT1, szString, sizeof(szString));
    WideCharToMultiByte(CP_ACP, 0, szString, -1, (LPSTR)szUUID, 16, NULL, NULL);
    ...
    for (i = 0; i < 16; i++)
    {
        ...
        {
            deviceId[i] = szUUID[i - nShift];
        }
    }

    if (!TOC_Read(g_pTOC, sizeof(g_TOC))) 
    {
        MessageBox(hWnd, TEXT("UUID Update Failed"), TEXT("Update UUID"), MB_OK);
        return 0;
    }

    memcpy(g_pTOC->deviceId, deviceId, 16);

    if (!TOC_Write(g_pTOC, sizeof(g_TOC))) 
    {
        MessageBox(hWnd, TEXT("UUID update failed"), TEXT("Update UUID"), MB_OK|MB_ICONQUESTION);
    }
    else
    {
        MessageBox(hWnd, TEXT("UUID update succeeded"), TEXT("Update UUID"), MB_OK|MB_ICONQUESTION);
    }
